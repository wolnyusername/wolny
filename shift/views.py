        ...
        if "user_id" in self.request.session:
            ...
            if (
                Shift.objects.filter(
                    worker=self.request.session.get("user_id"), end_time=None
                ).count()
                == 0
            ):
...
class UserStartShiftView(View):
    def get(self, request):
        new_shift = Shift(
            worker=models.Worker.objects.get(id=request.session["user_id"])
        )
        new_shift.save()
...
class UserEndShiftView(View):
    def get(self, request):
        Shift.objects.filter(
            worker=Worker.objects.get(id=request.session["user_id"]), end_time=None
